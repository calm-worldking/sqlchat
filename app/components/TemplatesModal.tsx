  ...
  const loadTemplates = () => {
    try {
      ...
      if (savedTemplates) {
        ...
        // Добавляем поле format к существующим шаблонам, если его нет
        const updatedTemplates = parsedTemplates.map((template: any) => ({
          ...template,
          format: template.format || 'text'
        }));
        setTemplates(updatedTemplates);
        // Сохраняем обновленные шаблоны обратно в localStorage
        localStorage.setItem('reportTemplates', JSON.stringify(updatedTemplates));
      } else {
        // Создаем шаблоны по умолчанию при первом запуске
